      ...
      <div className={`px-3 py-2 border-b transition-colors ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className={`text-xs font-medium px-1.5 py-0.5 rounded transition-all ${
              isDarkMode 
      ...
      <div className="p-3">
        ...
        >
          <div className="space-y-1">
            {statuses.filter(status => status.id).map((status, index) => (
              <React.Fragment key={status.id}>
                {/* Drop Placeholder - show at specific position for cross-category drops */}
                {shouldShowPlaceholder && isDraggedFromDifferentCategory && dragOverIndex !== null && dragOverIndex === index && (
                  <div className={`py-1.5 px-2 rounded border-2 border-dashed transition-all duration-200 ${
                    isDarkMode 
            ...
            {shouldShowPlaceholder && isDraggedFromDifferentCategory && dragOverIndex !== null && dragOverIndex >= statuses.length && (
              <div className={`py-1.5 px-2 rounded border-2 border-dashed transition-all duration-200 ${
                isDarkMode 
                  ? 'border-blue-400 bg-blue-900/20 text-blue-300' 
                  : 'border-blue-400 bg-blue-50 text-blue-600'
        ...
        {/* Add Status Form */}
        {showAddForm && (
          <div className={`mt-3 p-2 rounded border-2 border-dashed transition-all duration-200 ${
            isDarkMode 
...
const ManageStatusModal: React.FC<ManageStatusModalProps> = ({
  ...
  const handleCategoryChange = useCallback(async (id: string, categoryId: string, insertIndex?: number) => {
    ...
    try {
      ...
      if (insertIndex !== undefined) {
        ...
        const targetCategoryIndex = statusesByUpdatedCategory.findIndex(cat => cat.id === categoryId);
        if (targetCategoryIndex !== -1) {
          const targetCategory = statusesByUpdatedCategory[targetCategoryIndex];
                     ...
                     const otherStatuses = targetCategory.statuses.filter((s: IKanbanTaskStatus) => s.id !== id);
  ...
  const handleDragOver = useCallback((event: DragOverEvent) => {
    ...
    const draggedStatus = localStatuses.find(s => s.id === activeId) as IKanbanTaskStatus | undefined;
    if (!draggedStatus) {
      setDragOverCategory(null);
      setDragOverIndex(null);
      return;
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const draggedStatus = localStatuses.find(s => s.id === draggedStatusId) as IKanbanTaskStatus | undefined;
    ...
    >
      <div className="space-y-4">
        ...
        >
          ...
          <DragOverlay>
            {activeId ? (
              <div className={`py-1.5 px-2 rounded border transition-all duration-200 shadow-lg ${
                isDarkMode
