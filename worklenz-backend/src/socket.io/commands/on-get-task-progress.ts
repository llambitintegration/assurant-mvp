...
export async function on_get_task_progress(_io: Server, socket: Socket, taskId?: string): Promise<void> {
  try {
    ...
    socket.emit(SocketEvents.GET_TASK_PROGRESS.toString(), task);
    return;
  } catch (error) {
    log_error(error);
  }
