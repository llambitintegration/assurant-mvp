/**
 * Austin Cycle Count TSV Parser
 *
 * Parses the Austin location cycle count TSV file and generates JSON output for database import.
 * This script implements Agent 1 from the cycle count data import plan.
 *
 * Input:  /context/RobCo Inc Cycle Count - 12_19_25 - Austin.tsv (106 rows)
 * Output: /worklenz-backend/src/utils/data-migration/output/austin-cycle-count.json
 *
 * @module parse-austin-cycle-count
 */
 ...
 * TSV row structure
 */
interface AustinCycleCountRow {
  'RobCo Part Number': string; // SKU
  'Mfg. Part Number': string; // Manufacturer part number
 ...
 */
async function main() {
  console.log('=== Austin Cycle Count Parser ===\n');
  ...
  try {
    ...
    const inputFilePath = path.resolve(
      __dirname,
      '../../../../../context/RobCo Inc Cycle Count - 12_19_25 - Austin.tsv'
    );

    const rows = parseTsvFile(inputFilePath);
    const records = mapTsvToObjects(rows) as unknown as AustinCycleCountRow[];
    ...
    const components: ComponentRecord[] = validRecords.map((record) => {
      ...
      const partNumber = record['Mfg. Part Number']?.trim() || null;
    ...
    const outputData: OutputData = {
      ...
      components,
      _metadata: {
        source_file: 'RobCo Inc Cycle Count - 12_19_25 - Austin.tsv',
    ...
    const outputFilePath = path.resolve(
      __dirname,
      '../output/austin-cycle-count.json'
    );
    ...
    console.log('\n✓ Austin cycle count parsing completed successfully!');
  } catch (error) {
    console.error('\n❌ Error:', error);
    process.exit(1);
  } finally {
